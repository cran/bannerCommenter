---
title: "Consistent Banner Comments for R Scripts"
...
output:
  pdf_document:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Consistent Banner Comments for R Scripts}
...
This tiny package is purely for the convenience of authors who wish to
document their code with the occasional block of clearly marked
comments in order to make the code more easily navigable.
...
2. Rather than typing strings with quite delimiters separated by
   commas, if the function is called with no string arguments, and in
   an interactive session of course, the strings are read from the
   terminal, line by line, with prompts issued as with the `scan`
   function.  An empty line indicates the lines are complete.
...
decoration.  Comments are important, but excessive decoration can
detract from the aesthetics rather than enhance, and even with this
package making them can waste a lot of time.
